...
bool Socket::init(uint32_t sendTimeout, uint32_t recvTimeout)
{
    ...
    if(sendTimeout)
        setsockopt(m_socket, SOL_SOCKET, SO_SNDTIMEO, (char *)&sendTimeout, sizeof(DWORD));
    if(recvTimeout)
        setsockopt(m_socket, SOL_SOCKET, SO_RCVTIMEO, (char*)&recvTimeout, sizeof(DWORD));
